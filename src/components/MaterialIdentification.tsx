...
function identifyPeaks(spectrum: SpectrumBin[], totalAtoms: number): IdentifiedPeak[] {
  return ELEMENT_MAP.map(el => {
    ...
    const peakBin = bins.reduce(
      ...
      { mz: el.mzMin + (el.mzMax - el.mzMin) / 2, count: 0 },
    )
    return {
      label: el.label,
...
export function MaterialIdentification({ spectrum, totalAtoms, quality }: Props) {
  const peaks = identifyPeaks(spectrum, totalAtoms)

  const uniqueElements = [...new Map(peaks.filter(p => p.label !== '?').map(p => [p.label, p])).values()]
  const materialLabel = uniqueElements.length > 0
    ? `${uniqueElements.map(e => e.label).join('-')} Alloy`
